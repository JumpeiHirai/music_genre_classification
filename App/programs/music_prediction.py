...
def predict_genre(file_path):
    ...
    # モデルの読み込み
    model = tf.keras.models.load_model('../system/models/mfcc_model')
    ...
    plt.savefig('../system/predict.png')
    ...
    # 推定結果表示
    np.save('../system/genre_name',class_names[np.argmax(predictions[0])])
    return(class_names[np.argmax(predictions[0])])

# 入力が2つの時（推定時）実行
if len(sys.argv) == 2:
    if not os.path.exists('../system/models/mfcc_model'):
        if not os.path.exists('../system/datas/mfccs_dataset.npz'):
            import_data()
        trainer()
