...
def import_data():
    ...
    for foldername in os.listdir(path):
        for filename in os.listdir(path + '/' + foldername):
            ...
            try:
                ...
                # MFCC変換
                mfcc = librosa.feature.mfcc(y=music, sr=fs, n_mfcc=30)
                ...
                #min_frames = min(min_frames, mfcc.shape[1])
    ...
    mfccs = [np.resize(mfcc,(30,min_frames)) for mfcc in mfccs]
    # mfcc変換データ, ラベル, サンプリングレートを保存
    try:
        mfccs = np.array(mfccs)
        print(mfccs.shape)
        np.savez(os.path.join('./', 'mfccs_dataset'),mfccs,labels,max_fs)
    except Exception as e:
        print('SaveErr : Continue running')

# mfcc_dataset.npzがある場合、データセットの読み込み
def trainer():
    if os.path.exists('mfccs_dataset_added.npz'):
        data = np.load('mfccs_dataset_added.npz', allow_pickle=True)
    elif os.path.exists('mfccs_dataset.npz'):
    ...
    # モデルの保存
    try:
        if os.path.exists('mfcc_model'):
            model.save('mfcc_model_added')
        else:
