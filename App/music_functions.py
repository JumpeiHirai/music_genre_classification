...
def wsl_btn(file_path,x):
    ...
    subprocess.Popen(wsl_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def convert_to_wsl_path(win_path):
    command = f"wsl wslpath -u {win_path}"
...
def predict():
    ...
    genre_name = stdout.decode().strip()  

    return genre_name, file_path
